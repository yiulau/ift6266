...
errs=[]
iteration = 0
for r_testx in real_test_stream.get_epoch_iterator():
    if len(r_testx[0]) == batch_size:
        cur_err=predictions(r_testx[0]).flatten()
        for item in cur_err: 
            errs.append(item)
import pickle
f=open('realpredicitons','wb')
pickle.dump([errs],f)
f.close()
sys.exit()
a=0.5
...
for i in range(50):
    ...
    for inputx,inputy in train_stream.get_epoch_iterator():
        if len(inputy) == batch_size :
            o = train_model(inputx,inputy.flatten(),lr)
            train_errs.append(o[0])
            t_grad_norm.append(o[1])
            t_param_norm.append(o[2])
    print(sum(train_errs[-num_t_batches:])/num_t_batches)
    print(sum(t_grad_norm[-num_t_batches:])/num_t_batches)
    print(sum(t_param_norm[-num_t_batches:])/num_t_batches)
    ...
    epoch_valid_err = sum(valid_errs[-num_v_batches:])/num_v_batches
    print(epoch_valid_err)
    import pickle
    f=open('debugsave_7','wb')
